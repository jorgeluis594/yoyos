...
`User.companyId` referencia una `Company` y puede ser `null` mientras la cuenta termina el registro. Las tablas de autenticación son globales y no tienen RLS. Un resolvedor compartido valida la sesión de Better Auth y lee `User.companyId` con `systemPrisma`. El middleware de rutas tenant bajo `/api` usa ese ID, nunca uno enviado por el cliente: devuelve `401` sin sesión y `409` si falta empresa. Los loaders y actions protegidos deben usar el mismo resolvedor y envolver sus consultas tenant en `withTenantIsolation(companyId, callback)`.
...
`withTenantIsolation` guarda la empresa ya autorizada en `AsyncLocalStorage` y no abre una transacción.
...
`systemPrisma` es el cliente base para autenticación y consultas globales: no exige contexto Company y conserva las transacciones nativas que usa Better Auth. `prisma` exige contexto Company para todas sus operaciones y configura `app.company_id` dentro de cada transacción. La base de datos aplica RLS a `Company` incluso cuando se usa `systemPrisma`.
...
Para ejecutar la prueba de aislamiento en una base desechable, sigue [Validación de RLS](validacion-rls.md).
