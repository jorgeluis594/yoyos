...
import { action, loader } from "@core/app/routes/product-edit";
...
test("rejects manipulated non-editable fields before invoking the update", async () => {
  ...
  const request = new Request("http://localhost/es-PE/products/1/edit", { method: "POST", body: JSON.stringify({ currency: "PEN" }) });
...
test("surfaces expected update failures on the form and reports technical failures safely", async () => {
  ...
  const notFound = new Request("http://localhost/es-PE/products/1/edit", { method: "POST", body: JSON.stringify({ name: "Nuevo" }) });
  ...
  const request = new Request("http://localhost/es-PE/products/1/edit", { method: "POST", body: JSON.stringify({ name: "Nuevo" }) });
