...
describe("storeMessageImage", () => {
  it("downloads and imports before returning a ready image", async () => {
    ...
    expect(deps.importImage).toHaveBeenCalledWith("whatsapp-message:wamid.1", expect.objectContaining({ filename: "media-1" }));
  });
  ...
  it("records invalid images and unavailable storage as final failures", async () => {
    const deps = setup();
    vi.mocked(deps.importImage).mockResolvedValueOnce({ success: false, error: { code: "INVALID_IMAGE", message: "invalid pixels" } });
    ...
    vi.mocked(deps.importImage).mockResolvedValueOnce({ success: false, error: { code: "IMAGE_STORAGE_UNAVAILABLE", message: "storage down" } });
