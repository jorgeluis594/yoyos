...
describe("parseWhatsAppWebhook", () => {
  it("walks all entries and normalizes incoming and seller echo messages without trimming text", () => {
    const result = parseWhatsAppWebhook({ object: "whatsapp_business_account", entry: [
      { id: "waba-1", changes: [{ field: "messages", value: { metadata: { phone_number_id: "phone-1" }, contacts: [{ wa_id: "14155552671", profile: { name: "Sam" } }], messages: [{ id: "m1", from: "14155552671", timestamp: "10", type: "text", text: { body: " hi " } }], smb_message_echoes: [{ id: "m2", to: "14155552671", timestamp: "11", type: "image", image: { id: "media-1", caption: " caption " } }] } }] },
    ...
    expect(result?.events).toEqual(expect.arrayContaining([
      ...
      expect.objectContaining({ status: "message", message: expect.objectContaining({ content: { type: "image", mediaId: "media-1", caption: " caption " }, origin: { direction: "outgoing", source: "seller", userId: null } }) }),
