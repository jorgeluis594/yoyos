...
test("catalog searches distinct products and summarizes all variants within the company", async () => {
  ...
  const add = async (companyId: CompanyId, name: string, variants: { sku?: string; price: number; stock: number }[]) => {
    const id = randomUUID();
    await prisma.product.create({ data: { id, companyId, name, currency: "PEN", qrCode: randomUUID(), status: "active", createdAt: when, updatedAt: when } });
    for (const variant of variants) {
      const variantId = randomUUID();
      await prisma.productVariant.create({ data: { id: variantId, companyId, productId: id, attributes: {}, sku: variant.sku, salePrice: variant.price, qrCode: randomUUID(), status: "active" } });
      await prisma.productStock.create({ data: { companyId, variantId, quantity: BigInt(variant.stock) } });
    }
    return id;
  };
  try {
    ...
    const ids = await withTenantIsolation(companyA, async () => [
      await add(companyA, "Camisa", [{ sku: "MATCH-M", price: 20, stock: 2 }, { sku: "MATCH-L", price: 25, stock: 3 }, { sku: "OTHER", price: 30, stock: 4 }]),
      await add(companyA, "Match equal", [{ price: 10, stock: 1 }, { price: 10, stock: 2 }]),
      await add(companyA, "Unrelated", [{ sku: "SOLO", price: 5, stock: 0 }]),
    ]);
