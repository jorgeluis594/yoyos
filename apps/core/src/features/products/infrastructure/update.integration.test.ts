...
import type { ProductId, VariantId } from "@core/src/features/products/domain/product";
...
test("product updates persist effective changes, ignore no-ops, and roll back atomically", async () => {
  ...
  const getStored = async (companyId: string, id: ProductId) => {
  ...
  try {
    ...
    await withTenantIsolation(companyA, async () => {
      const created = await createProduct(base, createDeps);
      ...
      const edited = await updateProduct(id, {
      ...
      expect(await updateProduct(id, {}, deps)).toEqual({ success: true, data: id });
      expect(await updateProduct(id, { name: "Camisa nueva", variants: [{ id: medium.id, sku: "CAM-M-2" }] }, deps)).toEqual({ success: true, data: id });
      expect((await getStored(companyA, id)).updatedAt).toEqual(new Date("2026-12-01T00:00:00.000Z"));

      expect(await updateProduct(id, { description: "Nueva" }, deps)).toEqual({ success: true, data: id });
      const withDescription = await getStored(companyA, id);
      expect(withDescription.description).toBe("Nueva");
      expect(await updateProduct(id, { variants: [{ id: medium.id, purchasePrice: 0 }] }, deps)).toEqual({ success: true, data: id });
      ...
      const foreign = await updateProduct(id, { variants: [{ id: randomUUID() as VariantId, salePrice: 1 }] }, deps);
      expect(foreign).toMatchObject({ success: false, error: { code: "VALIDATION_ERROR" } });

      const rollback = await updateProduct(id, {
      ...
      const missing = await updateProduct(randomUUID() as ProductId, { name: "Fantasma" }, deps);
    ...
    await withTenantIsolation(companyB, async () => {
      const sameSku = await createProduct({ ...base, variants: [{ attributes: {}, sku: "CAM-M", salePrice: 5 }] }, createDeps);
      expect(sameSku).toMatchObject({ success: true });
      const absent = await createProduct({ ...base, variants: [{ attributes: { Talla: "M" }, salePrice: 5 }, { attributes: { Talla: "L" }, salePrice: 6 }] }, createDeps);
    ...
    await withTenantIsolation(companyA, async () => {
      expect(await updateProduct(foreignProductId!, { name: "Intruso" }, deps)).toMatchObject({ success: false, error: { code: "PRODUCT_NOT_FOUND" } });
      expect(await updateProduct(foreignProductId!, { variants: [{ id: foreignVariantId!, salePrice: 1 }] }, deps)).toMatchObject({ success: false, error: { code: "PRODUCT_NOT_FOUND" } });
    });
  } finally {
