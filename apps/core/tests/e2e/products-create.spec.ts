...
test("create products from the private form, validate input, and reload detail", async ({ page, request }) => {
  ...
  try {
    ...
    await page.getByRole("link", { name: "Nuevo producto" }).last().click();
    ...
    await browserExpect(page.getByRole("complementary").getByRole("link", { name: "Nuevo producto" })).toHaveAttribute("aria-current", "page");
    ...
    await browserExpect(page).toHaveURL(/\/dashboard$/);
    ...
    await browserExpect(page.getByText("Stock").locator("..")).toContainText(["2", "3"]);
