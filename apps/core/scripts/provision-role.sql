...
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname IN ('Company', 'Image', 'Product', 'ProductVariant', 'ProductStock', 'Contact', 'Chat', 'ChatMessage', 'user', 'session', 'account', 'verification', 'jwks') AND relowner = 'core_app'::regrole) THEN
...
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public."Company", public."Image", public."Product", public."ProductVariant", public."ProductStock", public."Contact", public."Chat", public."ChatMessage", public."user", public."session", public."account", public."verification", public."jwks" TO core_app;
COMMIT;
