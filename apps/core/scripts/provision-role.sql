...
BEGIN
  ...
  END IF;
  IF EXISTS (SELECT 1 FROM pg_class WHERE relname IN ('Company', 'user', 'session', 'account', 'verification') AND relowner = 'core_app'::regrole) THEN
...
GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE public."Company", public."user", public."session", public."account", public."verification" TO core_app;
COMMIT;
